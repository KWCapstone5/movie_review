...
def get_movie_info_kmdb(movie_title):
    ...
    if data['TotalCount'] > 0 and data['Data']:
        ...
        plot = plot_list[0]['plotText'] if plot_list else "정보 없음"
    
    return genre, keyword, plot

# CSV 파일 경로
input_file_path = './static/images/weekly_box_office_details.csv'  # 입력 파일 경로
output_file_path = './static/images/updated_weekly_box_office_details.csv'  # 출력 파일 경로
...
with open(input_file_path, mode='r', newline='', encoding='utf-8') as infile, open(output_file_path, mode='w', newline='', encoding='utf-8') as outfile:
    ...
    fieldnames = reader.fieldnames + ['genre', 'keyword', 'plot']  # 새로운 필드 추가
    writer = csv.DictWriter(outfile, fieldnames=fieldnames)
    writer.writeheader()
    
    for row in reader:
        # 영화 제목으로 정보 조회
        genre, keyword, plot = get_movie_info_kmdb(row['title'])
        # 조회한 정보를 행에 추가
        row.update({'genre': genre, 'keyword': keyword, 'plot': plot})
        writer.writerow(row)
